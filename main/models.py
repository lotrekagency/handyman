...
class Machine(models.Model):
    ...
    reseller = models.ForeignKey(
        Reseller, null=True, blank=True, on_delete=models.SET_NULL
    )
    administration_panel = models.CharField(max_length=200, null=True, blank=True)
    administration_panel_username = models.CharField(
        max_length=200, null=True, blank=True
    )
    administration_panel_password = models.CharField(
        max_length=200, null=True, blank=True
    )
...
class Domain(models.Model):
    ...
    price = models.DecimalField(max_digits=6, decimal_places=2, null=True, blank=True)
    end_time = models.DateField(null=True, blank=True)
    site_on = models.ManyToManyField(
        Machine, related_name="site", null=True, blank=True
    )
    site_on_backup = models.ForeignKey(
        Project,
    ...
    mail_on_backup = models.ForeignKey(
...
class Report(models.Model):
    ...
    class_type = models.CharField(
        max_length=4, choices=REPORT_TYPES, blank=True, null=True, verbose_name="Type"
    )
    ...
    def _get_report_titles(self, url):
        ...
        if self.class_type == REPORT_TYPE_BACK:
            return {
                "mail": "⚠️ #{0} Backup error for {1}".format(
                    self.pk, self.project.slug
                ),
        ...
        if self.class_type == REPORT_TYPE_TEST:
            return {
                "mail": "❌ #{0} Tests are failing for {1}".format(
                    self.pk, self.project.slug
                ),
        ...
        if self.class_type == REPORT_TYPE_DEADLINE:
            return {
                "mail": "⏰ #{0} New deadline for {1}".format(
                    self.pk, self.project.slug
                ),
        ...
        if self.class_type == REPORT_TYPE_MACHINE_DEADLINE:
            return {
                "mail": "🎰 #{0} New machine deadline for {1}".format(
                    self.pk, self.project.slug
                ),
    ...
    def notify(self):
        ...
        users_emails = LotrekUser.objects.filter(project=self.project).values_list(
            "email", flat=True
        )
        url = urllib.parse.urljoin(
            self.get_host(), reverse("admin:main_report_change", args=[self.id])
        )
        report_titles = self._get_report_titles(url)
        try:
            payload = {"text": report_titles["slack"]}
            requests.post(getattr(settings, "SLACK_WEBHOOK"), data=json.dumps(payload))
...
DEADLINE_TYPES = (("DOM", "Domain"), ("CERT", "Certificate"), ("OTHR", "Other"))
