...
def execute_backup(project):
    ...
    try:
        ...
        if password:
            ssh.connect(
                server, username=username, password=password, banner_timeout=200
            )
        else:
            key_path = os.path.expanduser(os.path.join("~", ".ssh", "id_rsa"))
            key = paramiko.RSAKey.from_private_key_file(
                key_path, os.getenv("RSA_KEY_PASSPHRASE")
            )
            ssh.connect(server, username=username, pkey=key, banner_timeout=200)
        print(script.split("\r\n"))
